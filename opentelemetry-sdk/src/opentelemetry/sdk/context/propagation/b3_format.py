...
class B3Format(HTTPTextFormat):
    ...
    def extract(cls, get_from_carrier, carrier):
        ...
        if single_header:
            ...
            trace_id = (
                _extract_first_element(get_from_carrier(carrier, cls.TRACE_ID_KEY))
                or trace_id
            )
            span_id = (
                _extract_first_element(get_from_carrier(carrier, cls.SPAN_ID_KEY))
                or span_id
            )
            sampled = (
                _extract_first_element(get_from_carrier(carrier, cls.SAMPLED_KEY))
                or sampled
            )
            flags = (
    ...
    def inject(cls, context, set_in_carrier, carrier):
        ...
        set_in_carrier(carrier, cls.TRACE_ID_KEY, format_trace_id(context.trace_id))
        set_in_carrier(carrier, cls.SPAN_ID_KEY, format_span_id(context.span_id))
