...
class TestB3Format(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.serialized_trace_id = b3_format.format_trace_id(trace.generate_trace_id())
        cls.serialized_span_id = b3_format.format_span_id(trace.generate_span_id())
    ...
    def test_extract_multi_header(self):
        ...
        self.assertEqual(new_carrier[FORMAT.TRACE_ID_KEY], self.serialized_trace_id)
        self.assertEqual(new_carrier[FORMAT.SPAN_ID_KEY], self.serialized_span_id)
    ...
    def test_extract_header_precedence(self):
        ...
        new_carrier = {}
        FORMAT.inject(span_context, dict.__setitem__, new_carrier)
        self.assertEqual(new_carrier[FORMAT.TRACE_ID_KEY], single_header_trace_id)
    ...
    def test_64bit_trace_id(self):
        ...
        new_carrier = {}
        FORMAT.inject(span_context, dict.__setitem__, new_carrier)
        self.assertEqual(new_carrier[FORMAT.TRACE_ID_KEY], "0" * 16 + trace_id_64_bit)
    ...
    def test_missing_trace_id(self):
        ...
        carrier = {FORMAT.SPAN_ID_KEY: self.serialized_span_id, FORMAT.FLAGS_KEY: "1"}
        span_context = FORMAT.extract(get_as_list, carrier)
        self.assertEqual(span_context.trace_id, api_trace.INVALID_TRACE_ID)

    def test_missing_span_id(self):
        """If a trace id is missing, populate an invalid trace id."""
        carrier = {FORMAT.TRACE_ID_KEY: self.serialized_trace_id, FORMAT.FLAGS_KEY: "1"}
        span_context = FORMAT.extract(get_as_list, carrier)
        self.assertEqual(span_context.span_id, api_trace.INVALID_SPAN_ID)
